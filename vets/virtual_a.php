...
if (isset($_SESSION['phone_no'])) {
    ...
    $q = "SELECT virtual_appointments.*, vets.v_id, users.u_id
    FROM virtual_appointments
    JOIN vets ON virtual_appointments.v_id = vets.v_id
    JOIN users ON virtual_appointments.u_id = users.u_id
...
if (!$result) {
    ...
    echo '<form action="v_appointment.php" method="post">';
    echo '<table>';
    echo '<thead>';
    echo '<tr>';
    ...
    foreach ($rows as $row) {
        ...
        if (isset($row['vir_id'])) {
            echo '<button type="submit" name="accept" value="' . $row['vir_id'] . '">Accept</button>';
            echo '<button type="submit" name="reject" value="' . $row['vir_id'] . '">Reject</button>';
